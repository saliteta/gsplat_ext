...
class GaussianPrimitive(Primitive):
    ...
    def filtering_alpha(self, alpha_threshold: Union[float, None] = None, percentage: Union[float, None] = None) -> None:
        """
        Filtering the splats that has low alpha value
        """
        if alpha_threshold is None and percentage is None:
            raise ValueError("Either alpha_threshold or percentage must be provided")
        if alpha_threshold is not None:
            splat_mask = self.geometry["opacities"] > alpha_threshold
        else:
            threshold_value = torch.quantile(self.geometry["opacities"], 1.0 - percentage)
            splat_mask = self.geometry["opacities"] > threshold_value
        return splat_mask
